# 🛠️ Contributing to Jirnyak/selo_temi
...
> Technology Foundation: `JavaScript / Canvas 2D / Markov Generator`
...
Selo Temi Procedural Rural Topography & Slavic Folklore is engineered for maximum performance, deterministic state transitions, and zero computational slop. All contributions must respect existing subsystem boundaries and data flows:

```mermaid
graph TD
    A[Perlin Noise Terrain] --> B[Hydrological River Descent]
    B --> C[Slavic Settlement Placer]
    C --> D[Markov Folklore Narrative Engine]
```
...
### 1. Contiguous River Descent
* **Formal Requirement**: Elevation gradient descent must generate contiguous, non-looping river paths.
...
### 2. Markov Grammar Phonotactics
* **Formal Requirement**: Procedural name generation must respect Slavic phonetic grammar constraints.
* **Verification Protocol**: Automated unit test assertion + mathematical boundary check.
* **Failure Mode**: Immediate build rejection; PR cannot be approved without meeting this invariant.
...
* Primary Runtime: `JavaScript / Canvas 2D / Markov Generator`
...
```typescript
// ✅ CORRECT: River Gradient Descent
function findRiverStep(grid: Float32Array, w: number, x: number, y: number): [number, number] {
    let minH = grid[y * w + x], nx = x, ny = y;
    for (const [dx, dy] of [[-1,0],[1,0],[0,-1],[0,1]]) {
        const h = grid[(y + dy) * w + (x + dx)];
        if (h < minH) { minH = h; nx = x + dx; ny = y + dy; }
    }
    return [nx, ny];
}
```
